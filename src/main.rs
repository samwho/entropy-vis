...
fn main() -> Result<(), Error> {
    ...
    let mut grid_width = 80;
    let mut grid_height = 24;
    ...
    match (args.width, args.height) {
        ...
        _ => {
            if let Some((term_width, term_height)) = terminal_size() {
                grid_width = max(1, term_width.0 as u64 / 2);
                grid_height = max(1, (term_height.0 as f64 / 1.1).ceil() as u64);
            }
    ...
    loop {
        ...
        for byte in buf.iter().take(bytes_read) {
            ...
            if count == chunk_size {
                ...
                let c = Rgb(intensity, intensity, intensity);
                write!(stdout, "{}", "  ".on_color(c))?;
                sum = 0;
                count = 0;
                total_chunks += 1;
